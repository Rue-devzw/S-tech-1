...
export default function AdminListings() {
  ...
  const [editing, setEditing] = useState<import("@/lib/mock-data").Listing | null>(null)
  const [dialogOpen, setDialogOpen] = useState(false)

  function handleSave(item: import("@/lib/mock-data").Listing) {
    if (editing) {
  ...
  function openEdit(item: import("@/lib/mock-data").Listing) {
    setEditing(item)
    setDialogOpen(true)
  }
    ...
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <Tabs defaultValue="all" className="w-full" onValueChange={setActiveTab}>
        <ListingForm
          ...
          trigger={<></>} // invisible trigger, we control externally
        />
                    ...
                    <TableCell className="text-right pr-6">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-40 rounded-xl">
                          ...
                          <DropdownMenuItem className="py-2.5 flex items-center gap-2 cursor-pointer" onClick={() => openEdit(item)}>
                            <Edit3 className="w-4 h-4" /> Edit Listing
                          </DropdownMenuItem>
                          <DropdownMenuItem className="py-2.5 flex items-center gap-2 cursor-pointer text-destructive focus:text-destructive focus:bg-destructive/5" onClick={() => handleDelete(item.id)}>
